...
CREATE SEQUENCE CONSTRACT_GOODS INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE INVENTORY_MANAGEMENT INCREMENT BY 1 START WITH 1;
...
VALUES(BOARD_SEQ.NEXTVAL, 'SYS_45282', '5월 공지사항 제목1', '5월 공지사항 내용1', 1, '2022-05-30','2023-07-30',0, 'Y');

--게시글 수정
UPDATE BOARD SET TITLE='5월 공지사항 제목 수정', CONTENT='5월 공지사항 내용 수정' WHERE SEQ = '1' AND EMP_CODE='SYS_45282';
...
--게시판 상세조회
SELECT SEQ, TITLE, CONTENT, STARTDATE, ENDDATE, S_COUNT, POST_USE
	FROM BOARD2 
		WHERE SEQ='1';
	

-- 게시판 카운트 
UPDATE BOARD SET S_COUNT = S_COUNT+1 WHERE EMP_CODE != 'SYS_45281' AND SEQ ='1';

--게시글 전체 조회 (사용자는 삭제된글 보기 불가, 관리자는 삭제된 글 보기 가능) EMP_CODE: SYS_45282 /  POST_USE : Y or N
SELECT SEQ, EMP_CODE, TITLE, CONTENT, IMPORTANT, STARTDATE, ENDDATE, S_COUNT, POST_USE
 	FROM BOARD
 	WHERE POST_USE ='Y';
...
--게시기간이 시작되면  POST_USE 를 Y으로 변경
UPDATE BOARD SET POST_USE ='N' 
	WHERE TO_DATE(SYSDATE, 'YYYY-MM-DD') = TO_DATE(STARTDATE, 'YYYY-MM-DD');
	
 --게시기간이 끝나면 POST_USE 를 N으로 변경
 UPDATE BOARD SET POST_USE ='Y' 
	WHERE TO_DATE(SYSDATE, 'YYYY-MM-DD') = TO_DATE(ENDDATE, 'YYYY-MM-DD');
